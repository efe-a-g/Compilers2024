...
let ruleset replace =
  function
    (* Merge and simplify offset calculations *)
    ...
    (* Loads and stores combined with addressing *)
    | GLOBAL x :: LOADW :: _ ->
        replace 2 [LDGW x]
    | GLOBAL x :: STOREW :: _ ->
        replace 2 [STGW x]
...
(* |rewrite| -- iterate over the code and apply rules until converged *)
