/* Action code -- generated by iset.tcl */
 ...
 sp++;
          ...
          ACTION(BOUND_2)
               pc = pc0 + 3;
               check((unsigned) sp[1].i >= (unsigned) sp[0].i, E_BOUND, (get2(pc0+1))); sp++;
               NEXT;

          ACTION(NCHECK_2)
               pc = pc0 + 3;
               check(pointer(sp[0]) == NULL, E_NULL, (get2(pc0+1)));
               NEXT;

          ACTION(GCHECK_2)
               pc = pc0 + 3;
               check(valptr(sp[0]) != NULL, E_GLOB, (get2(pc0+1))); sp += 1;
               NEXT;

          ACTION(ZCHECK_2)
               pc = pc0 + 3;
               check(sp[0].i == 0, E_DIV, (get2(pc0+1)));
               NEXT;

          ACTION(FZCHECK_2)
               pc = pc0 + 3;
               check(sp[0].f == 0.0, E_FDIV, (get2(pc0+1)));
               NEXT;

          ACTION(DZCHECK_2)
               pc = pc0 + 3;
               check(get_double(&sp[0]) == 0.0, E_FDIV, (get2(pc0+1)));
               NEXT;

          ACTION(QZCHECK_2)
               pc = pc0 + 3;
               check(get_long(&sp[0]) == 0, E_DIV, (get2(pc0+1)));
               NEXT;
