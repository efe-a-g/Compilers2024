...
and check_libcall q args env v =
  (* |q.q_argtypes = []| if the argument types can vary; otherwise
     it is a list of arg types. *)
  if List.length args <> q.q_nargs then
...
(* |make_def| -- construct a definition *)
let make_def x k t =
...
let param_alloc pcount d =
  let s = Mach.param_rep.r_size in  
  d.d_addr <- Local (param_base + s * !pcount);
  pcount := !pcount + param_size d
...
and check_decl d env = 
  ...
  match d with
    ...
    | ProcDecl (Heading (x, _, _) as heading, body) ->
        ...
        d.d_addr <- Global (proc_symbol x.x_name);
  ...
  let check =
    function
        ProcDecl (Heading(x, _, _), body) ->
          ...
          incr level; name_stack := x.x_name :: !name_stack;
          check_block d.d_level p.p_result env' body;
          decr level; name_stack := List.tl !name_stack
      | _ -> () in
  List.iter check ds
...
(* |libproc| -- definition for library procedure *)
let libproc i n ts = 
  LibDef { q_id = i; q_nargs = n; q_argtypes = ts }

(* |operator| -- definition for library procedure defined by instruction *)
let operator op ts =
  libproc (Operator op) (List.length ts) ts

(* |init_env| -- environment for whole program *)
let init_env = 
  let defn (x, k, t) env = define (make_def (intern x) k t) env in
  Util.accum defn
